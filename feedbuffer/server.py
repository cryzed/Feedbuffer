...
class Server:
    ...
    def index(self, url, update_interval=DEFAULT_UPDATE_INTERVAL):
        ...
        if feed.update_interval != update_interval:
            logger.info('Changing update interval from %d to %d seconds for feed: %s',
        ...
        response = core.generate_feed(feed.data, [entry.data for entry in feed.entries], feed.encoding)
